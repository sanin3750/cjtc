...
public class BackgroundController extends BaseController {
	...
	public String index(Model model) throws Exception {
		...
		//查询用户对应角色
		UserGroupsFormMap usFormMap = new UserGroupsFormMap();
		usFormMap.put("userId", userFormMap.get("id"));
		//查询用户对应角色
		List<UserGroupsFormMap> ugFormMapList = resourcesMapper.findByNames(usFormMap);
		
		StringBuffer _roleIds = new StringBuffer();
		for(UserGroupsFormMap ug : ugFormMapList){
			_roleIds.append(ug.get("roleId")).append(",");
		}
		Map<String, Object> paramMap = new HashMap<String, Object>();
		String roleIds = _roleIds.toString();
		paramMap.put("roleIds", roleIds.substring(0, roleIds.lastIndexOf(",")));
		List<ResFormMap> mps = resourcesMapper.findUserRoleRes(paramMap);
